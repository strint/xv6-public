...
struct {
  ...
  struct inode inode[NINODE];  // 50个active inode
} icache;  // cache of inode
...
ialloc(uint dev, short type)
{
  ...
  for(inum = 1; inum < sb.ninodes; inum++){
    bp = bread(dev, IBLOCK(inum, sb));  // 读取inum对应的dinode所在的block
    dip = (struct dinode*)bp->data +  // 起始地址转成dinode类型指针
        inum%IPB;  // 偏移量
    // type 为0的是空inode
    if(dip->type == 0){  // a free inode
      ...
      brelse(bp);
      return iget(dev, inum); // 返回在icache中的inode指针
    }
    brelse(bp);
  }
...
// 用memory中的inode去更新其对应的dinode并写到Disk
...
iput(struct inode *ip)
{
  ...
  if(ip->valid && ip->nlink == 0){
    ...
    if(r == 1){
      ...
      itrunc(ip);  // 
      ip->type = 0;  // icache中对应inode被释放
      iupdate(ip);  // 更新dinode到磁盘
      ip->valid = 0;
    }
  }
  releasesleep(&ip->lock);
...
// 返回inode的第i个数据block对应的block num(没有时做block的分配)
//PAGEBREAK!
// Inode content
//
...
static uint
bmap(struct inode *ip, uint bn)  // block map
{
  uint addr, *a;
  struct buf *bp;
...
// 从inode的数据off位置开始读n个byte，放到dst中
// 磁盘到内存
//PAGEBREAK!
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    ...
    // min(剩余要读的, 起始位到一个block结尾的大小)
    // 处理剩余要读小于一个block或者起始位不是一个block起点的情况
    m = min(n - tot, BSIZE - off%BSIZE);
    // memmove实际做的是byte级的copy
...
// 从inode的数据off位置开始写n个byte，数据源在src
// 内存到磁盘
// PAGEBREAK!
// Write data to inode.
// Caller must hold ip->lock.
...
dirlookup(struct inode *dp, char *name, uint *poff)
{
  ...
  // 每次读inode数据的16byte（一个dirent的大小）
  for(off = 0; off < dp->size; off += sizeof(de)){
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
      panic("dirlookup read");
    if(de.inum == 0)
      continue;
    // 如果dir name能匹配上，就找到了对应的dir entry
